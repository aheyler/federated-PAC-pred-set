...
# class FMNISTParticipantDataset: 
...
#                 save_path = f"/home/aheyler/PAC-pred-set-DP-v1-working/snapshots/{args.exp_name}/participants_arr"
#                 np.save(save_path, sampled_participants)
...
#                             # Create datasets
#                             participant_dataset_train = FMNISTParticipantDataset(x_train, y_train, participant, transform=tforms_dft, image_shape=(28,28))
#                             participant_dataset_val = FMNISTParticipantDataset(x_val, y_val, participant, transform=tforms_dft, image_shape=(28,28))
#                             participant_dataset_test = FMNISTParticipantDataset(x_test, y_test, participant, transform=tforms_dft, image_shape=(28,28))
...
test_labels = np.load("/home/aheyler/PAC-pred-set/data/femnist/f0000_14/test_labels.npy")
print(test_labels)
test_images = np.load("/home/aheyler/PAC-pred-set/data/femnist/f0000_14/test_images.npy")

for i in range(62): 
    if i in test_labels: 
        idx = test_labels == i
        test_image = test_images[idx] * 255
        print(test_image.shape)
        print(type(test_image))
        print(test_image)
        # data = im.fromarray(np.array([test_image, test_image, test_image]))
        data = im.fromarray(test_image)
        data.save(f"{i}.png")
        break
    break