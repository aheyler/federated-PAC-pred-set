...
def init_fft_conv(weight, hw):
    ...
    complex = fft_to_complex_matrix(torch.fft.fft(kernel, 2))
    print(f"init_fft_conv output shape: {complex.shape}")
    return complex
    # return fft_to_complex_matrix(torch.fft.rfft(kernel, 2), onesided=False))
...
def fft_conv(x, w_fft, transpose=False):
    ...
    """
    x_fft = fft_to_complex_vector(torch.fft.fft(x, 2)) # onesided=False))
    ...
    return torch.fft.ifft(wx_fft, 2) #, onesided=False)
...
class MONMultiConv(nn.Module):
    ...
    def init_inverse(self, alpha, beta):
        ...
        print(f"A[1] shape: {self.A(1).shape}")
        print(f"A[2] shape: {self.A(2).shape}")
        print(f"B[0] shape: {self.B(0).shape}")
        print(f"B[1] shape: {self.B(1).shape}")
        print(f"B[2] shape: {self.B(2).shape}")
        print(f"Conv[0] shape: {(self.conv_shp[0], self.conv_shp[0])}")
        print(f"Conv[1] shape: {(self.conv_shp[0], self.conv_shp[0])}")
        print(f"Conv[2] shape: {(self.conv_shp[0], self.conv_shp[0])}")
    ...
    def apply_inverse_conv(self, z, i):
        z0_fft = fft_to_complex_vector(torch.fft.fft(z, 2)) # onesided=False))
        ...
        x0 = torch.fft.ifft(x0, 2) # onesided=False)
        return x0

    def apply_inverse_conv_transpose(self, g, i):
        g0_fft = fft_to_complex_vector(torch.fft.fft(g, 2)) #onesided=False))
...
# linear_module = MONMultiConv(in_channels=3, conv_channels=(8,16,32), 
# image_size=34, kernel_size=3, m=1.0)


# #The module knows the correct shape for its hidden states z_i at each tier for a given batch size
# n_batch = 4
