...
##
class PredSetFederated(nn.Module):
...
class PredSetFederatedCls(PredSetFederated):
    """
    T \in [0, \infty]
    """
    def __init__(self, mdl, eps=0.0, delta=0.0, n=0):
    ...
    def membership(self, x, y):
        # print(f"T in cls membership: {self.T}")
        with tc.no_grad():
            s = self.set(x, y)
            # print(f"s = {s}")
...
class PredSetRegFederated(PredSetFederated):
    """
    ...
    def membership(self, x, y):
        # print(f"T in reg membership: {self.T}")
        with tc.no_grad():
